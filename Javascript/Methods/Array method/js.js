/*
    ...
    splice(vị_trí_đặt_con_trỏ, 0:chèn/1:thay_đổi, 'Chuỗi_muốn_chèn_vào') : chèn hoặc thay đổi một phần tử cho mảng
...
var squaredNumbers = numbers_map.map(function (x) {
  // x: đại diện từng phần tử trong mảng numbers
  return x * x;
});
...
numbers_fil.filter(function (x) {
  // x: đại diện từng phần tử trong mảng numbers_fil
...
numbers_find.find(function (x) {
  // x: đại diện từng phần tử trong mảng numbers_find
...
numbers_reduce.reduce(function (a, b) {
  // lấy 1 + 2 = 3 sau đó lấy 3 + 3 = 6 sau đó lấy 6 + 4 = 10
  return a + b;
});
...
var products_reduce = products_map.reduce(function (
  products_map1,
  products_map2
) {
...
/*
  Includes (sử dung cho: string, array): giúp kiểm tra sự tồn tại của item bên trong mảng/chuỗi
*/
...
/* 
  Array.from(): Cho phép tạo các array từ một kiểu dữ liệu khác,
*/
const name_arr = "javascript";
const nameArray = Array.from(name_arr);
console.log(name_arr); // => javascript
console.log(nameArray); // => ['j', 'a', 'v', 'a', 's', 'c', 'r', 'i', 'p', 't']
//---------------------------------------------------------------------------------------

/* 
  Array.prototype.entries(): Cho phép tạo các array từ một kiểu dữ liệu khác,
*/
var arr = ["a", "b", "c"];
